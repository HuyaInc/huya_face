...
def resnet(data, units, num_stages, filter_list, num_classes, bottle_neck):
    ...
    print(version_se, version_input, version_unit, act_type, memonger, name_prefix)
    num_unit = len(units)
    assert(num_unit == num_stages)
    
    body = data
    body = Conv(data=body, num_filter=filter_list[0], kernel=(3,3), stride=(1,1), pad=(1, 1),
                              no_bias=True, name="{}conv0".format(name_prefix), workspace=workspace)
    body = mx.sym.BatchNorm(data=body, fix_gamma=False, eps=2e-5, momentum=bn_mom, name='{}bn0'.format(name_prefix), cudnn_off=True)
    body = Act(data=body, act_type=act_type, name='{}relu0'.format(name_prefix))

    for i in range(num_stages):
      body = residual_unit(body, filter_list[i+1], (2, 2), False,
        name='%sstage%d_unit%d' % (name_prefix, i + 1, 1), bottle_neck=bottle_neck, **kwargs)
      for j in range(units[i]-1):
        body = residual_unit(body, filter_list[i+1], (1,1), True, name='%sstage%d_unit%d' % (name_prefix, i+1, j+2),
          bottle_neck=bottle_neck, **kwargs)

    if bottle_neck:
      body = Conv(data=body, num_filter=512, kernel=(1,1), stride=(1,1), pad=(0,0),
                                no_bias=True, name="{}convd".format(name_prefix), workspace=workspace)
      body = mx.sym.BatchNorm(data=body, fix_gamma=False, eps=2e-5, momentum=bn_mom, name='{}bnd'.format(name_prefix), cudnn_off=True)
      body = Act(data=body, act_type=act_type, name='{}relud'.format(name_prefix))
...
def get_symbol(data):
    ...
    print('====name prefix : ', config.name_prefix, ' layer : ', num_layers)
    if num_layers >= 500:
        assert False
