...
def transpose_channel(image, mod='rgb'):
    size = image.shape
    out = np.zeros((size))
    if mod == 'rgb':
        out = image
...
class FaceImageIter(io.DataIter):
    def __init__(self, batch_size, data_shape,
                 ...
                 images_filter = 0, color_shuffle = False,
        ...
        self.channel_mods = ['rgb', 'rbg', 'bgr', 'brg', 'gbr', 'grb']
        logging.info('=== set color channel shuflle {}'.format(color_shuffle))
        print('= color shuffle ', color_shuffle)
    ...
    def next(self):
        ...
        try:
            while i < batch_size:
                ...
                if self.color_shuffle:
                    mod_idx = random.randint(0, 5)
                    mod = self.channel_mods[mod_idx]
                    _data = nd.array(transpose_channel(_data.asnumpy(), mod))
                ...
                for datum in data:
                    ...
                    #print("=== label ", label, ' type(label) ', type(label), " batch_label ", batch_label)
                    batch_label[i][:] = label
                    i += 1
